...
from Apriori import findsubsets, frequent_itemsets, apriori_gen, has_infrequent_subset, L1, D, min_sup, confidence
...
def upload_files():
    return render_template('Min_sup.html')
...
def script():
    if request.method == 'POST':
        global min_sup
        min_sup= int(request.form['min_sup'])/10
    op = generate_association_rules()
    return render_template("Output.html", output=op)
